...
CustomController::CustomController(Effector waterPump, Effector sirupPump, Effector waterValve, Effector sirupValve, Effector heater, 
    Sensor colour, Sensor temprature, Sensor level, Sensor cup, LEDGreen LedGreen, LEDYellow LedYellow, LCD lcd, Keypad Keypad):
    waterPump(waterPump), waterValve(waterValve), sirupPump(sirupPump), sirupValve(sirupValve), heater(heater), colour(colour), temperature(temperature), level(level),
    LedGreen(LedGreen), LedYellow(LedYellow), lcd(lcd), keypad(keypad)
{
    state = States::IDLE;
    fault = Faults::NONE;
    beginLevelCup = 0.0;
    ...
    PrettyProgressIcon progress = PrettyProgressIcon();
...
void CustomController::updateDisplay(void){
    ...
    if(buffer <= 100){
        lcd.pushString('(' + progress.get() + ')' + buffer + "%");
    }else{
        lcd.pushString("      Done!      ");
    }
}
