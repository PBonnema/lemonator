...
class Controller:
    ...
    def update(self) -> None:
        ...
        self.LCDDisplay.clear()
        self.LCDDisplay.pushString("\x0c      Lemonator\n--------------------\n")
        ...
        if self.state == States.WAITING_FOR_USER_ACTION:
            if self.Cup.readValue():
                ...
                self.LCDDisplay.pushString("Please place a cup\nto continue...\n")
        ...
        if self.state == States.DISPENSING:
            fromTargetPercentage = int((self.Level.readValue() * 100.0) / self.targetLevel)

            self.LCDDisplay.pushString("Dispensing... (" + str(fromTargetPercentage) + "%)")
